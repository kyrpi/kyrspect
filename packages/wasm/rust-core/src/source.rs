...
pub fn analyze_source(url: &str, mime_type_hint: Option<&str>) -> SourceInfo {
    ...
    let (media_type, probable_mime) = if let Some(mime) = mime_type_hint {
        ...
        if m.contains("x-mpegurl") || m.contains("vnd.apple.mpegurl") || m.contains("application/dash+xml") {
    ...
    let is_stream = media_type == MediaType::Hls || media_type == MediaType::MediaStream;
